...
<div class="container my-5 py-5">
    <div class="row">
        <div class="col-12">
            <h2 class="display-5 py-5 text-seccondary">{{__("Resultados de la búsqueda")}}: {{$q}}</h2>
        </div>
        @foreach ($ads as $ad)
                ...
                <div class="card-body d-flex flex-column justify-content-around">
                    ...
                    <h6 class="card-subtitle my-1">
                        <strong>Categoría: <a
                                ...
                                class="links">{{$ad->category->name}}</a></strong>
                    </h6>
                    <a href="{{route('ad.details', $ad->slug)}}" class="btn mb-0 my-4 btn-acent links">{{__("Más")}}</a>
                </div>
            </div>
        </div>
        @endforeach
